...
def evaluate_gates(features: dict[str, Any], market_rs_rating: float | None) -> dict[str, bool | None]:
    """Apply price gates while leaving broad-market RS explicit when unavailable."""
...
def apply_momentum(features_by_id: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    for key, feature in features_by_id.items():
        ...
        item["portfolio_rs_rating"] = rating
        item["rs_rating"] = rating
        item["market_rs_rating"] = None
        gates = evaluate_gates(item, item["market_rs_rating"]) if item.get("sma200_21d_ago") is not None else {}
        item["gates"] = gates
        item["gates_passed"] = sum(value is True for value in gates.values())
        item["stage2"] = None if gates and any(value is None for value in gates.values()) else (bool(gates) and all(gates.values()))
        item["stage2_status"] = "market-rs-unavailable" if gates and any(value is None for value in gates.values()) else ("ready" if gates else "unavailable")
